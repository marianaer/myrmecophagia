...
for i in $(cat Hsunycteris_thomasi_muc5b_isoforms.list);do grep -E $i"(\^|\s)" kallisto.isoform.TPM.not_cross_norm_annotated >>  kallisto_annotated_muc5b.results; done
...
awk '{ print $1"_MUC7_BLAST "$2; }' kallisto_annotated_muc7.results >> kallisto.isoform.TPM.not_cross_norm_annotated
